...
const UserProfile = ({ login, email, token }) => {
  ...
  const changePassword = async (newPassword) => {
    ...
    try {
      const response = await fetch('http://46.8.64.99:8000/api/change-password/', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        console.error(`Ошибка при изменении пароля: ${response.status} ${response.statusText}`);
      }
    } catch (error) {
